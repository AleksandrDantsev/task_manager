...
const isCompletedTask = computed(() => {
    return store.getters.getStateIsCompletedTask([props.date, props.id]);
})

const levelSubtasks = computed(() => {
    return store.getters.getLevelSubtasks([props.date, props.id]);
})

const stageFillCompletedTask = computed(() => {
    return Math.round(levelSubtasks.value[0] / levelSubtasks.value[1] * 100);
})
...
const doneTask = () => {
    store.commit("doneTask", [props.date, props.id]);
}
    ...
    <div>
        ...
        <div class="task_card" :class="isCompletedTask ? 'completed-task ' + `${level}-color-border` : `${level}-color-border`">
            ...
            <div v-if="levelSubtasks[1] > 0" class="completed-subtasks">
                <div class="level-completed-subtasks">
                    <div class="line-fill" :style="{'width': stageFillCompletedTask + '%'}"></div>
                </div>
                <div class="completed-subtasks-numbers">{{ levelSubtasks[0] }} / {{ levelSubtasks[1] }}</div>
            </div>
        </div>
    </div>
</template>
